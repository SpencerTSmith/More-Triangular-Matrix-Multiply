...
extern "C" void compute_device(int m0, int n0, float *A_distributed,
                               float *B_distributed, float *C_distributed);

extern "C" void allocate_device(int m0, int n0, float **A_device,
                                float **B_device, float **C_device);

extern "C" void free_device(int m0, int n0, float *A_device, float *B_device,
                            float *C_device);

extern "C" void collect_data_from_device(int m0, int n0, float *C_device,
                                         float *C_distributed);

extern "C" void distribute_data_to_device(int m0, int n0, float *A_distributed,
                                          float *B_distributed, float *A_device,
                                          float *B_device);

#define BLOCK_SIZE 32

/* This is the GPU kernel. */
__global__ void cuda_trmm(int m0, int n0, float *A_device, float *B_device,
                          float *C_device) {
  /*
  ...
  */
  ...
  // same as simple matrix multiply kernel... only with the added j0 > i0
  // condition
  if ((i0 < m0 && j0 < n0) && j0 > i0) {
    float res = 0.0f;
    for (int p0 = 0; p0 < m0; ++p0) {
      float A_ip = A_device[i0 + p0 * rs_A];
      float B_pj = B_device[p0 + j0 * rs_B];
...
void allocate_device(int m0, int n0, float **A_device, float **B_device,
                     float **C_device) {
...
void distribute_data_to_device(int m0, int n0, float *A_distributed,
                               float *B_distributed, float *A_device,
                               float *B_device) {
  int bytes_A = m0 * m0 * sizeof(float);
  int bytes_B = m0 * n0 * sizeof(float);
  ...
  gpuErrchk(
      cudaMemcpy(A_device, A_distributed, bytes_A, cudaMemcpyHostToDevice));
  gpuErrchk(
      cudaMemcpy(B_device, B_distributed, bytes_B, cudaMemcpyHostToDevice));

  // printf("Done\n");
}

void collect_data_from_device(int m0, int n0, float *C_device,
                              float *C_distributed) {
  int bytes_C = m0 * n0 * sizeof(float);
  ...
  gpuErrchk(
      cudaMemcpy(C_distributed, C_device, bytes_C, cudaMemcpyDeviceToHost));
  // printf("Done\n");
}

void free_device(int m0, int n0, float *A_device, float *B_device,
                 float *C_device) {
  // Free GPU memory
  // printf("GPU Free: ");
...
void compute_device(int m0, int n0, float *A_distributed, float *B_distributed,
                    float *C_distributed)

{
  ...
  dim3 block_dim(BLOCK_SIZE, BLOCK_SIZE); // Threads per block
  dim3 grid_dim((m0 + BLOCK_SIZE - 1) / BLOCK_SIZE,
                (n0 + BLOCK_SIZE - 1) / BLOCK_SIZE); // Number of blocks

  // Run the kernel
  cuda_trmm<<<grid_dim, block_dim>>>(m0, n0, A_distributed, B_distributed,
                                     C_distributed);
